...
def get_wsi_patch(filename, patch_size=256, downsample=[1], augment=0):
    ...
    region, mask, x_start, y_start = get_patch(wsi, xml_path, class_num, l_dims, level, slide_mask, patch_size, filename, downsample, augment)
...
def get_patch(wsi, xml_path, annotationID, l_dims, level, slide_mask, patch_size, filename, downsample, augment):
    ...
    while True:
        ...
        if level == -1: # if no resolution works return white region
            ...
            return np.ones((patch_size, patch_size,3), dtype=np.uint8)*255, np.zeros((patch_size, patch_size), dtype=np.uint8), 0, 0
...
def get_grid_list(slide_path, patch_size, downsample, wsi=None, overlap_num=4):
    ...
    slide_mask = get_slide_mask(slide_path)
